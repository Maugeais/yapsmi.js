...
var fret_positions = [2.54, 9.297, 16.05, 22.377, 28.441, 34.262, 39.741, 44.981, 49.90, 54.664, 59.003, 63.317, 67.243, 71.058, 74.668, 78.081, 81.375, 84.342, 87.183, 90.014, 92.555, 95.033, 97.367];
...
function losses_tau0_change(a, s){
  let display = inst.strings[s-1].set_control({"tau0" : a})
  $("#losses_tau0€"+s+"_value").html(display)

}

function losses_tau1_change(a, s){
  let display = inst.strings[s-1].set_control({"tau1" : a})
  $("#losses_tau1€"+s+"_value").html(display)
}
...
function id_string_fret(e){
  ...
  let string = 6-Math.floor(6*(e.clientY-$("#fretboard").offset().top)/e.target.offsetHeight);
...
function mute_string(s, state){
  if (state == false){
      ...
      $($(".stringName")[6-s]).css('color', 'lime');
  } else {
      inst.strings[s-1].muted = true;
      $($(".stringName")[6-s]).css('color', 'red');
  }
}
...
$(".stringName").dblclick(function(){
  let index = 6-$('.stringName').index(this);
...
$("#struming").mousedown(function(e){struming=true; var rect = e.target.getBoundingClientRect(); position_struming_y=(6*(e.clientY-rect.top)/$("#struming").height()-1/2);});
