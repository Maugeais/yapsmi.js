...
function init_continuation3d_analyser(audioCtx, uid){
  ...
  return({'input': analysers[uid], 'output': analysers[uid], 'is_on': true, 'callback': continuation3d_analyser})
}


function continuation3d_analyser(uid){
  ...
  Plotly.extendTraces('continuation3d_display', update, [0], tail_size)
}
...
window.continuation3d_tail_change = function(a){
  history.set_from_percentage(a);
  tail_size = history.value*48000/(2*2048);
  $("#continuation3d_tail_value").html(history.to_string());
}

console.log("I'm hare")

window.continuation3d_change_axis = function(axis, container){
  ...
  Plotly.extendTraces('continuation3d_display', update, [0], 0)
}

// let canvas = document.getElementById("continuation3d_display");
...
function draw(){
    ...
    Plotly.newPlot('continuation3d_display', data, layout, {displayModeBar: false});
// [{
//         type: 'scatter3d',
//         mode: 'lines',
//         x: x,
...
let continuation3d_knobs;
function init(uid){

  continuation3d_knobs = init_knobs("continuation3d_controls", "large", "Vintage");

  key_x = Object.keys(instrument_controls)[0];
  
  let keys = Object.keys(instrument_controls);
  $.each(keys, function (i, item) {
      $('#continuation3d_x').append($('<option>', {
          value: item,
          text : item
      }));
      $('#continuation3d_y').append($('<option>', {
          value: item,
          text : item
      }));
      $('#continuation3d_z').append($('<option>', {
          value: item,
          text : item
      }));
  });
  ...
  draw();
  add_filter(init_continuation3d_analyser, -1, uid); 
...
function load(uid, commands){    
  ...
  $("#continuation3d_x").val(key_x);
  key_y = commands['key_y'];
  $("#continuation3d_y").val(key_y);
  key_y = commands['key_y'];
  $("#continuation3d_y").val(key_y);
  continuation3d_knobs["continuation_tail"].setValue(commands["tail_size"])
